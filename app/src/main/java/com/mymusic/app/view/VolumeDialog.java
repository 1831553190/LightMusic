...
import com.mymusic.app.controller.VolumeDialogController.State;
import com.mymusic.app.controller.VolumeDialogController.StreamState;
import com.mymusic.app.util.Events;
import com.mymusic.app.util.Util;
import com.mymusic.app.util.VolumePrefs;
...
//    private Callback mCallback;
    ...
    public void setSeekPro(int v,int p) {
        mRows.get(p).slider.setFocusable(true);
        int pro=mRows.get(p).slider.getProgress();
        if (mRows.size()==2){
            ValueAnimator valueAnimator=ValueAnimator.ofInt(pro,v);
            valueAnimator.setInterpolator(new DecelerateInterpolator());
            valueAnimator.setDuration(200);
            valueAnimator.addUpdateListener(animation -> {
                mRows.get(p).slider.setProgress ((int) animation.getAnimatedValue());
...
//            Log.d("progresswtese", "animTest: "+wtest);
            ...
            mRows.get(p).slider.setProgress(v);
        }
    }

    public int idx(){
        if (mRows.get(1).slider.isFocused()){
            return 1;
        }
        return 0;
    }
...
//    public VolumeDialog(Context context, int windowType, VolumeDialogController controller, Callback callback) {
//        mContext = context;
...
//    }
    ...
    public interface SCallBack{
        void seekChange(VolumeRow row);
        void onProgressChanged(VolumeRow row, SeekBar seekBar, int progress, boolean fromUser);
        void onStopTrackingTouch(VolumeRow row, SeekBar seekBar,int progress);
        void onStartTrackingTouch(VolumeRow row, SeekBar seekBar);
    }

    public void setOnSeekProgressChange(SCallBack sCallBack){
        this.sCallBack=sCallBack;
    }

    public void initSeekBar(int p1,int p2){
        if (mRows.size()==2){
            mRows.get(0).slider.setProgress(p1);
            mRows.get(1).slider.setProgress(p2);
        }
    }
    public boolean getSeekIsMax(){
        if (mRows.size()==2){
            if (mRows.get(0).slider.getProgress()==mRows.get(0).slider.getMax()){
                mRows.get(1).slider.setFocusable(true);
                setExpandedH(true);
                return true;
            }
        }
    ...
    public VolumeDialog(Context context, VolumeDialogController controller, int windowType, SCallBack sCallBack) {
        mContext = context;
        mController = controller;
        this.sCallBack=sCallBack;
//        mCallback = callback;
        ...
        addRow(0,
                R.drawable.ic_volume_media, R.drawable.ic_volume_media, true);
        addRow(1,
                R.drawable.ic_baseline_phone_android_24, R.drawable.ic_volume_ringer_mute, true);
...
//    VolumeDialog(Context context, int windowType) {
//        mContext = context;
//        mSpTexts = new SpTexts(mContext);
//        mKeyguard = (KeyguardManager) context.getSystemService(Context.KEYGUARD_SERVICE);
//        mAudioManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
...
//                    }
    ...
    private final class VolumeSeekBarChangeListener implements OnSeekBarChangeListener {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (sCallBack!=null) {
                sCallBack.onProgressChanged(mRow,seekBar, progress/10, fromUser);
            }
            ...
            final int userLevel = getImpliedLevel(seekBar, progress/10);
        ...
        public void onStartTrackingTouch(SeekBar seekBar) {
            if (sCallBack!=null){
                sCallBack.onStartTrackingTouch(mRow,seekBar);
            }
        ...
        public void onStopTrackingTouch(SeekBar seekBar) {
            if (sCallBack!=null){
                sCallBack.onStopTrackingTouch(mRow,seekBar,seekBar.getProgress()/10);
            }
            mRow.tracking = false;
            mRow.userAttempt = SystemClock.uptimeMillis();
